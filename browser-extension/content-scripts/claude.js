// Claude Content Script - Intercepts messages before sending to Claude.ai
...
function initializeInterceptor() {
  ...
  for (const selector of selectors) {
    ...
    for (const input of inputs) {
      if (input.offsetHeight > 30 || input.className.includes('ProseMirror')) {
...
function findSendButton() {
  // Claude's send button - multiple selectors for robustness
  const selectors = [
  ...
  // Fallback: find button with send-like aria-label
...
// Retry initialization periodically (Claude is a SPA)
setInterval(() => {
